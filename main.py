...
class Message:
  ...
  def tuple_unique(self) -> tuple[str, int, str]:
    match self.operation, *self.args:
      case ("SEARCH", mode, *_):
        return adress_string(self.origin), self.seqno, f"SEARCH_{mode}"
      case (op,):
        return adress_string(self.origin), self.seqno, op
...
class Stats:
  ...
  def record_fl_value(self, hops: int):
    self.fl_avg_hops += hops
    self.fl_avgsqr_hops += hops * hops
    self.fl_val_count += 1
  
  def record_rw_value(self, hops: int):
    self.rw_avg_hops += hops
    self.rw_avgsqr_hops += hops * hops
    self.rw_val_count += 1

  def record_bp_value(self, hops: int):
    self.bp_avg_hops += hops
    self.bp_avgsqr_hops += hops * hops
    self.bp_val_count += 1

  def fl_stats_calc(self) -> str:
    if self.fl_val_count == 0:
      return "N/A"
    avg = self.fl_avg_hops / self.fl_val_count
    avgsqr = self.fl_avgsqr_hops / self.fl_val_count
    var = avgsqr - avg * avg
    dev = var ** 0.5
    return f"{avg:.3} (dp {dev:.3})"
  
  def rw_stats_calc(self) -> str:
    if self.rw_val_count == 0:
      return "N/A"
    avg = self.rw_avg_hops / self.rw_val_count
    avgsqr = self.rw_avgsqr_hops / self.rw_val_count
    var = avgsqr - avg * avg
    dev = var ** 0.5
    return f"{avg:.3} (dp {dev:.3})"

  def bp_stats_calc(self) -> str:
    if self.bp_val_count == 0:
      return "N/A"
    avg = self.bp_avg_hops / self.bp_val_count
    avgsqr = self.bp_avgsqr_hops / self.bp_val_count
    var = avgsqr - avg * avg
    dev = var ** 0.5
    return f"{avg:.3} (dp {dev:.3})"
...
class Node:
  ...
  def has_key_locally(self, key: str) -> bool:
  ...
  async def initiate_server(self):
    ...
    server = await asyncio.initiate_server(self.process_client_connection, addr, port)
    async with server:
